...
function App(){
  ...
  function statsFor(ids:number[]){const idSet=new Set(ids);const records=attempts.filter(a=>idSet.has(a.countryId));const correct=records.filter(a=>a.correct).length;return {attempted:records.length,correct,rate:records.length?Math.round(correct/records.length*100):null}}
  ...
  return <main>
    {screen==='home'&&<section className="home">
      ...
      <div className="setup">
        ...
        <div className="region-summary"><span>{region}の学習記録</span><b>{selectedRegionStats.rate===null?'まだ記録がありません':`正答率 ${selectedRegionStats.rate}%`}</b><small>{selectedRegionStats.attempted}回回答・{selectedRegionStats.correct}回正解</small></div>
